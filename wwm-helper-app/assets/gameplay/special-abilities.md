# Special Abilities & Mechanics

## Touch of Death (暗杀 - Assassination)

### Overview
**Type**: General Mystic Skill (stealth kill mechanic)

**Vitality Cost**: 0 (free to use)

**Function**: Execute enemies or deal massive damage without being detected

**Mechanics**:
- Charge toward target from behind
- Strike vital point on enemy's back with finger
- **Instant kill on weak enemies** (weak constitutions)
- **Massive damage on strong enemies**

**Requirements**:
- Enemy must be unaware (use Veil of Stillness)
- Approach from behind or when enemy is blinded

**Unlock**: Side Story: "Volume 1 - The Realm Endures"

**Upgrade**: Can be upgraded (Ebon Iron + Beauty's Plume)
- Increases damage with each tier
- Allows kills on enemies with larger HP pools

### Related Mystic Skills & Inner Ways

**Veil of Stillness** (敛息):
- Reduces enemy detection
- Makes it easier to approach for Touch of Death
- Cannot be used in combat
- Must approach from behind/sides

**Blinding Mist** (迷雾):
- Blinds enemies for 5 seconds
- Can use Touch of Death on blinded targets
- Tier 3: +20% Touch of Death damage during effect

**Shadow Assault** (Inner Way):
- +1.5m Touch of Death range
- +10% Touch of Death damage
- Restores 5% of damage dealt as HP

### Uses
- Stealth eliminations of guards/patrols
- Massive damage to unaware enemies
- Non-alert takedowns
- Essential for stealth builds

---

## Windsense (风感 - Wind Perception)

### Overview
**Type**: General Mystic Skill (detection and perception ability)

**Vitality Cost**: 0 (free to use)

**Function**: See enemy patrol routes, field of view, and enable special interactions

**Mechanics**:
- Perceive approaching entities through wind currents
- Distinguish position by sensing surroundings
- **Highlights items or people of interest** in area
- Activated ability/toggle

**Unlock**: Main Quest (automatic)

**Upgrade**: Cannot be upgraded

### Comprehensive Uses

**Combat**:
- Quickly spot enemies
- Identify interactable objects
- View enemy **strengths and weaknesses**
- Initiate **Healing mode**

**Stealth**:
- **Identify field of view of patrolling enemies** (detection cones)
- See enemy detection zones
- Plan stealth routes
- Essential with Veil of Stillness

**NPC Interaction**:
- See what NPCs are carrying
- Identify items to steal with Celestial Seize
- View negotiation opportunities
- Enables stealing from NPCs (with Celestial Seize)

**Quests**:
- Track targets
- Find hidden objectives
- Reveal quest items
- Highlight important people/items

**General Rule**: When in doubt, use Windsense to spot something overlooked

### Related Skills

**With Veil of Stillness**:
- See enemy fields of view while stealthed
- Plan routes to avoid detection
- Navigate patrol patterns

**With Celestial Seize**:
- Enables stealing from NPCs
- Reveals stealable items
- Shows interaction opportunities

**With Touch of Death**:
- Identify unaware enemies
- Plan assassination routes
- See enemy positions

---

## Weak Point Break System

### Overview
Many Mystic Skills have effects that trigger "on Weak Point Break"

### What is a Weak Point Break?
**Status**: Mechanics TBD

**Likely Involves**:
- Hitting specific weak points on enemies
- Breaking enemy stance or guard
- Critical hit system
- Timing-based mechanic

### Skills with Weak Point Break Effects

**Tai Chi**:
- Tier 2: +20% Shield Breaker damage on Weak Point Break
- Tier 3: -20% target defense for 10s after Weak Point Break
- Tier 4: +30% additional Shield Breaker damage

**Meridian Touch**:
- Tier 2: +20% bonus damage on Weak Point Break
- Tier 3: +20% Qi damage taken by target for 10s
- Tier 4: +30% additional bonus damage

**Celestial Seize**:
- Tier 2: +20% bonus damage on Weak Point Break
- Tier 3: -20% target attack for 10s
- Tier 4: +30% additional bonus damage

**Cloud Steps**:
- Tier 2: +20% bonus damage on Weak Point Break
- Tier 3: +20% HP damage taken by target for 10s
- Tier 4: +30% additional bonus damage

---

## Endurance System

### Overview
**Endurance** is a resource separate from Health, Chi, Stamina, and Vitality

### What Endurance Does
- Affects enemy and player stamina
- Related to defensive actions
- Can be drained to create vulnerabilities

### Endurance Mechanics

**Endurance Recovery**:
- Characters naturally recover Endurance
- Can be reduced by skills

**Exhausted State**:
- Occurs when Endurance is depleted
- Mentioned in Serene Breeze Tier 4

### Skills Affecting Endurance

**Serene Breeze**:
- On hit: reduces target Endurance recovery by 50% for 8s
- Tier 4: Recovers 6 Qi if you're not Exhausted

**Ghostly Steps**:
- Tier 2: Reduces Endurance cost of ground dodge by 40%
- Tier 3: +10% Endurance reduction for all skills

**Talon Strike**:
- Tier 3: Reduces Endurance Recovery by 50% for 5s on defense break

---

## Defense Break System

### Overview
Some skills can "break enemy defense"

### What is Defense Break?
**Likely Involves**:
- Breaking through blocking enemies
- Stunning or staggering
- Creating vulnerability window
- Guard break mechanic

### Skills with Defense Break

**Talon Strike**:
- Swipe to break defense
- Tier 3: On defense break, reduces Defense by 10% and Endurance Recovery by 50% for 5s

**Yaksha Rush**:
- Break enemy defense and knock upward
- **Breaking defense deals great damage**
- Tier 3: Inflicts Silence for 3s on defense break

---

## Poise System

### Unstable Poise
Certain enemy types have "Unstable Poise" that makes them vulnerable to specific attacks

### Vulnerable Enemy Types
- **Cavalry** (mounted enemies)
- Other units with Unstable Poise designation

### Effective Skills Against Unstable Poise

**Cloud Steps**:
- Stomp on enemy shoulders
- Especially effective against Cavalry and Unstable Poise units

**Wolflike Frenzy**:
- Knock down with staff
- Effective against Cavalry and Unstable Poise units

---

## Shield System

### Shield Types

**HP Shield**:
- Temporary hit point buffer
- Can be granted by skills
- Absorbs damage before health

**Golden Body Shield**:
- 8-second duration
- Provides Fortitude and Damage Reduction while active
- Tier 2: Converts 50% of expired shield value to HP

### Skills Providing Shields

**Golden Body**:
- Grants Shield and Qi Recovery
- Tier 3: Can grant AoE shield to nearby allies (if Vitality ≥100)

**Free Morph**:
- Tier 3: +20% damage to targets with HP Shield

---

## Status Effect Immunity

### Mighty Enemies
"Mighty enemies" are immune to certain control effects

**Immune to**:
- Immobilization (Meridian Touch, Ghost Bind)
- Blinding (Blinding Mist)
- Other crowd control effects

**Not Immune to**:
- Damage
- Weak Point Breaks
- Debuffs (likely)

### Players
- Immune to Immobilization from Meridian Touch and Ghost Bind
- Immune to Blinding from Blinding Mist

---

## Fortitude & Tenacity

### Fortitude
**Effect**: Status effect resistance or stance stability

**Granted by**:
- Lion's Roar (Tier 2 enhances Tenacity effect)
- Golden Body (while Shield holds)
- Dragon's Breath (Tier 2 enhances Tenacity effect)

### Tenacity
**Effect**: Related to Fortitude, prevents interruption

**Notes**: May be the same as Fortitude or a related effect

---

## Super Armor

### Overview
**Effect**: Cannot be interrupted or staggered during certain skills

### Skills Granting Super Armor

**Flaming Meteor**:
- Gain Super Armor during skill execution

**Dragon Head**:
- Gain Super Armor during skill execution

**Serene Breeze**:
- Tier 2: Auto-break control and gain Super Armor briefly after 10s in Arena

---

## Damage Reduction Buffs

### Overview
Many Mystic Skills provide temporary damage reduction

### Skills with Damage Reduction

**15% Damage Reduction**:
- Lion's Roar
- Flaming Meteor
- Dragon's Breath
- Dragon Head
- Drunken Poet (Tier 2)

**40% Damage Reduction** (Upgraded):
- Lion's Roar (Tier 4)
- Flaming Meteor (Tier 4)
- Dragon's Breath (Tier 4)

**Golden Body**:
- Base Damage Reduction (while Shield holds)
- Tier 2: +10% Damage Reduction

---

## Lightness Skills

### Overview
"Lightness skills" are movement/mobility abilities in wuxia tradition

### What are Lightness Skills?
- Movement abilities
- Dashes and mobility techniques
- Likely includes dodges
- Can be prevented by debuffs

### Hamstring Debuff
**Effect**: Unable to cast lightness skills for 5 seconds

**Applied by**:
- Wolflike Frenzy (Tier 3)

---

## Campaign-Challenge Mode

### Overview
Special challenge mode for skill progression

### Features
- Deflection-based gameplay
- Build proficiency in skills
- Unlock certain Mystic Skills

### Skill Unlocks
**Flaming Meteor**:
- "Gain proficiency by deflecting in Campaign-Challenge"
- Acquire skill when Proficiency is full

---

## Insight Resource System

### Overview
**Insight** is a resource used for **Star Shift** (deflection mechanic)

### Visual Indicator
- Displayed as split bar under Qi bar during combat
- More splits = More Insight available
- Each segment represents 1 use of Star Shift

### How to Use
- **Star Shift** consumes Insight to trigger Deflection
- Neutralizes enemy attacks that cannot be countered
- Essential defensive resource

### Increasing Insight Cap

**Method**: Upgrade Star Shift Mystic Skill

**Upgrades**:
- Tier 2: +1 Insight Cap
- Tier 3: +1 Insight Cap (total +2 from base)

**Upgrade Materials**: Ebon Iron + Wine Jujube

### Strategy
- Start with limited Insight (base amount TBD)
- Upgrade Star Shift early for more deflections per fight
- Use for attacks that can't be countered or dodged
- Essential for defensive builds and boss fights

---

## Divine Counter & Exhaustion System

### Exhaustion State

**What is Exhaustion**:
- State when enemy or player Endurance is depleted
- Creates vulnerability window
- Enables special finishing attacks

**How to Cause**:
- Drain target's Endurance through repeated attacks
- Use skills that reduce Endurance recovery
- Relentless pressure

### Divine Counter Mechanic

**Skill**: Divine Counter (General Mystic Skill)

**Effect**:
- Counter-attack after defending
- Deals damage
- Reduces target Endurance recovery by 50% for 8s

**Strategy**:
- Use Divine Counter repeatedly to drain enemy Endurance
- Faster path to Exhaustion state
- Chain with other Endurance-draining skills

### Exhaustion Execution

**Vital Leech** (Inner Way):
- Casting Exhaustion Execution Skill restores 8% of damage dealt as HP
- Life steal from finishing attacks

**Strategy**:
- Drain Endurance with Divine Counter, Serene Breeze, Talon Strike
- Wait for Exhaustion state
- Perform high-damage finishing attack
- Restore HP with Vital Leech (if equipped)

---

## Oddities & Melodies of Peace

### Oddities System

**What are Oddities**:
- Collectible items scattered throughout world
- Used to increase max Vitality
- Essential progression for Mystic Skills

**Types of Oddities** (from Still Waters description):
- **Moonbeam Toads** - Mobile creatures
- **Midnight Nibblers** - Mobile creatures
- Other types TBD

### Collecting Oddities

**Challenges**:
- Some Oddities move and must be caught
- Use **Still Waters** to slow them down (75% movement reduction)
- Makes collection easier

### Melodies of Peace System

**Purpose**: Exchange Oddities for max Vitality increases

**Process**:
1. Collect Oddities throughout the world
2. Submit Oddities to Melodies of Peace system
3. Increase max Vitality
4. Unlock ability to cast high-cost Mystic Skills

**Importance**:
- Some Offensive Mystic Skills have high Vitality costs
- Starting Vitality insufficient for all skills
- Must increase cap through this system
- Essential for endgame builds

**Status**: Full mechanics TBD (how many Oddities, where to submit, exact Vitality increases)

---

## Data Collection Needs

**NEEDED INFORMATION**:
- Complete Touch of Death mechanics and damage
- Windsense activation and full capabilities
- Weak Point Break system explanation
- Endurance stat details and thresholds
- Defense Break exact mechanics
- Poise system full explanation
- Shield value calculations
- Fortitude/Tenacity exact effects
- Super Armor duration and specifics
- Complete status effect immunity list
- Lightness skill complete list
- Campaign-Challenge mode details
- Proficiency system mechanics
